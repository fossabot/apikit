...
public class FlowMetadata implements MetadataSource {
  ...
  public FlowMetadata(final EndPoint endPoint, final Operation operation, final ApiCoordinate coordinate,
  ...
  private MetadataType getInputPayload(final Operation operation, final ApiCoordinate coordinate) {
    ...
    return payload.map(p -> loadMetadata(p, coordinate, "input")).orElseGet(MetadataFactory::defaultMetadata);
  }
  ...
  private MetadataType getOutputPayload(final Operation operation, final ApiCoordinate coordinate) {
    ...
    return payload.map(p -> loadMetadata(p, coordinate, "output")).orElseGet(MetadataFactory::defaultMetadata);
  }

  private static ObjectTypeBuilder getOutputHeaders(final Operation operation) {
  ...
  private static ObjectTypeBuilder getInputHeaders(final Operation operation) {
    final ObjectTypeBuilder builder = BaseTypeBuilder.create(MetadataFormat.JAVA).objectType();
  ...
  private ObjectTypeBuilder getUriParameters(final EndPoint endPoint, final Map<String, Parameter> baseUriParameters) {
    ...
    baseUriParameters.forEach((name, param) -> builder.addField().key(name).value(metadata(param)));
  ...
  private static MetadataType metadata(final Parameter parameter) {
    try {
      return MetadataFactory.from(parameter.schema());
    } catch (Exception ignore) {
      System.out.println("MetadataFactory error for " + parameter.name() + " (" + parameter.schema().getClass() + ")");
    }
    return MetadataFactory.defaultMetadata();
  }

  private static void addFields(final ObjectTypeBuilder builder, final EndPoint endPoint, final List<Parameter> parameters) {
    ...
    if (!parameters.isEmpty()) {
      ...
      //final String s = keys.stream().map(Object::toString).collect(Collectors.joining(", "));
      //System.out.println(">>>>> FlowMetadata.getUriParameters >>>>>>> " + path + " len=" + keys.size() + " " + s);
  ...
  /*
  private static void addField(final ObjectTypeBuilder builder, final Parameter parameter) {
    builder.addField().key(parameter.name().value()).value(MetadataFactory.from(parameter));
  }
  */
  ...
  private MetadataType loadMetadata(final Payload payload, ApiCoordinate coordinate,
